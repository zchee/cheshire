...
var ArticleImageListItem = React.createClass({
  ...
  render: function () {
    if (!this.props.image.link || typeof(this.props.image.link) === 'string') {
      return (
        <div>Upload
          <div><label>Image<input type="file" name={this.props.name + '[image_attributes][link]'}
                                  value={this.props.image.link} onChange={this.props.onImageLinked}/></label></div>
          <div><label>Slug<input type='text' name={this.props.name + '[image_attributes][slug]'}
                                 value={this.props.image.slug} onChange={this.props.onImageSlugChange}/></label></div>
          <div><label>Position<input type="text" name={this.props.name + '[position]'} value={this.props.position}
                                     onChange={this.props.onPositionChange}/></label></div>
        </div>
      );
    }
    return (
...
var ArticleImageList = React.createClass({
  ...
  render: function () {
    ...
    var extraSlugs = this.props.extraSlugs.map(function (slug, index) {
      return <input key={'slug_' + index} type='text' name={'add_slug[' + index + ']'} value={slug}
                    onChange={this.props.onExtraSlugChange.bind(null, index)}/>;
    }, this);
    return (
      <div>
        <div>
...
var ArticleForm = React.createClass({
  ...
  isFormUploadingFiles: function () {
    ...
    for (i = 0; i < len; i++) {
      if (!this.props.articleImageAssociations[i].id) {
        return true;
      }
    }
    return false;
  ...
  render: function () {
    ...
    return (
      <form acceptCharset="UTF-8" action={formPath} method="post"
        ...
        <div><label>Slug<input type='text' name={this.props.name + '[slug]'} value={this.props.slug}
                               onChange={this.props.onSlugChange}/></label></div>
        <div><label>Headline<input type='text' name={this.props.name + '[headline]'} value={this.props.headline}
                                   onChange={this.props.onHeadlineChange}/></label></div>
        <div><label>Subheadline<input type='text' name={this.props.name + '[subheadline]'}
                                      value={this.props.subheadline} onChange={this.props.onSubheadlineChange}/></label>
        </div>
        <div><label>Contributors<textarea name={this.props.name + '[contributors]' } value={this.props.contributors}
                                          onChange={this.props.onContributorsChange}/></label></div>
        <div><label>Date<input type='text' name={this.props.name + '[date]'} value={this.props.date}
                               onChange={this.props.onDateChange}/></label></div>
        <div><label>Body<textarea name={this.props.name + '[body]'} value={this.props.body}
                                  onChange={this.props.onBodyChange}/></label></div>
        <div><label>State<select name={this.props.name + '[state]'} value={this.props.state}
                                 onChange={this.props.onStateChange}>{this.props.stateOptions}</select></label></div>
        <ArticleImageList
